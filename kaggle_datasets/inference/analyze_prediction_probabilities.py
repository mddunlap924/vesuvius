...
def predict_optimal_threshold(probs: np.ndarray, method: str = "learned") -> float:
    ...
    Updated for nnUNetTrainer_FineTune500epochs_LR001 model (2026-02-01):
    Key discriminating features between 0.15 and 0.20 optimal thresholds:
    - prob_mean: 0.15 optimal @ ~0.097, 0.20 optimal @ ~0.202
    - prob_kurtosis: 0.15 optimal @ ~5.42, 0.20 optimal @ ~1.14
    - prob_skewness: 0.15 optimal @ ~2.62, 0.20 optimal @ ~1.35
    - decisive_ratio: 0.15 optimal @ ~0.56, 0.20 optimal @ ~0.10
    - bimodality_coef: 0.15 optimal @ ~0.94, 0.20 optimal @ ~0.68
    ...
    if method == "learned":
        # Use learned decision boundaries from analysis of FineTune500epochs_LR001 model
        # Based on observed group statistics (2026-02-01):
        # - 0.15 optimal: prob_mean~0.097, kurtosis~5.42, skewness~2.62, decisive_ratio~0.56
        # - 0.20 optimal: prob_mean~0.202, kurtosis~1.14, skewness~1.35, decisive_ratio~0.10

        # Score towards high threshold (0.20)
        # The key indicator is high prob_mean with low kurtosis (flat, non-peaky distribution)
        high_score = 0
        if prob_mean > 0.15:  # Much higher than typical 0.097
            high_score += 3
        if prob_mean > 0.18:  # Even stronger signal
            high_score += 2
        if prob_kurtosis_val < 2.0:  # Flat distribution (typical is 5.4)
            high_score += 2.5
        if prob_kurtosis_val < 3.0:
            high_score += 1
        if prob_skewness_val < 1.8:  # More symmetric (typical is 2.6)
            high_score += 1.5
        if decisive_ratio < 0.25:  # Less confident predictions
            high_score += 1.5
        if bimodality_coef < 0.75:  # Less bimodal
            high_score += 1

        # Score towards default threshold (0.15)
        default_score = 0
        if prob_mean < 0.12:  # Typical range for 0.15 threshold
            default_score += 2
        if prob_kurtosis_val > 4.0:  # Peaky distribution
            default_score += 2
        if prob_skewness_val > 2.3:  # Highly skewed
            default_score += 1.5
        if decisive_ratio > 0.35:  # More confident predictions
            default_score += 1.5
        if bimodality_coef > 0.85:  # More bimodal
            default_score += 1

        # Decide based on scores
        if high_score >= 6 and high_score > default_score + 3:
            threshold = 0.20
        elif high_score >= 4 and high_score > default_score + 1:
            threshold = 0.18
        elif default_score >= 4 and default_score > high_score:
            threshold = 0.15
        else:
            # Default to 0.15 as it works for most samples
            threshold = 0.15

    elif method == "regression":
        # Linear regression based on key features
        # Updated normalization parameters for new model:
        # prob_mean: mean=0.118, std=0.048
        z_prob_mean = (prob_mean - 0.118) / 0.048
        # prob_kurtosis: mean=4.57, std=2.11
        z_kurtosis = (prob_kurtosis_val - 4.57) / 2.11
        # prob_skewness: mean=2.37, std=0.56
        z_skewness = (prob_skewness_val - 2.37) / 0.56
        # decisive_ratio: mean=0.47, std=0.26
        z_decisive = (decisive_ratio - 0.47) / 0.26

        # Baseline is 0.16 (weighted average: 4*0.15 + 1*0.20 = 0.16)
        threshold = 0.16
        threshold += 0.02 * z_prob_mean  # Higher prob_mean -> higher threshold
        threshold -= 0.015 * z_kurtosis  # Lower kurtosis -> higher threshold
        threshold -= 0.01 * z_skewness  # Lower skewness -> higher threshold
        threshold -= 0.01 * z_decisive  # Lower decisive -> higher threshold

    elif method == "rule_based":
        # Simple rule-based system from observed patterns
        # Sample optimal at 0.20: prob_mean~0.202, kurtosis~1.14, skewness~1.35
        # Samples optimal at 0.15: prob_mean~0.097, kurtosis~5.42, skewness~2.62

        if prob_mean > 0.15 and prob_kurtosis_val < 2.5:
            threshold = 0.20
        elif prob_mean > 0.13 and prob_kurtosis_val < 3.5:
            threshold = 0.18
        else:
            threshold = 0.15

    elif method == "hybrid":
        # Combine regression with rule-based constraints
        ...
        threshold = 0.16
        threshold += 0.02 * z_prob_mean
        threshold -= 0.015 * z_kurtosis
        threshold -= 0.01 * z_skewness
        threshold -= 0.01 * z_decisive

        # Apply rule-based boundaries based on strong signals
        if prob_mean > 0.15 and prob_kurtosis_val < 2.5:
            threshold = max(threshold, 0.18)
        elif prob_mean < 0.11 and prob_kurtosis_val > 4.5:
            threshold = min(threshold, 0.16)

    else:
        threshold = 0.15

    # Clamp to valid range (updated for new model - no 0.08 threshold needed)
...
if __name__ == "__main__":
    ...
    SCORES_CSV = BASE_DIR / "scoring/temp_results/comprehensive_results_20260131_103355.csv"
